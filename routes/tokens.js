...
/**
 * POST /api/tokens
 * Create a new managed token (superadmin only)
 */
 ...
 */
router.post('/:id/blacklist', authenticateToken, requireSuperAdmin, async (req, res) => {
  try {
    const { id } = req.params;
 ...
 */
router.delete('/:id/blacklist/:walletAddress', authenticateToken, requireSuperAdmin, async (req, res) => {
  try {
